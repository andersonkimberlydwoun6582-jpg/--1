...
import { buildJoinQuantStrategy, type StrategyConfig } from "./joinquant-strategy";

export type Strategy = "A股多因子优化版" | "价值成长" | "趋势择时" | "低波红利" | "五福融合改";
...
const lessons = [
  ...
  { n: "06", title: "AI 生成策略的对比实验", desc: "比较模型、平台和提示上下文，先验证再扩展", state: "已看完", color: "violet" },
  { n: "07", title: "策略要持续迭代：从回测到小资金实盘", desc: "理解滑点、成交量和维护成本，别把回测当承诺", state: "已看完", color: "sand" },
  { n: "08", title: "先做可运行的小轮子，再组装策略", desc: "比较 QMT、PTrade 与通达信，并处理重复下单风险", state: "已看完", color: "blue" },
  { n: "09", title: "从零写策略：先验证 API，再接入基本面过滤", desc: "用股东人数函数拆轮子，连接双均线与回测模拟", state: "已看完", color: "mint" },
];
...
function WufuCaseStudy() { return <section className="wufu-case-study"><div className="wufu-case-head"><div><p className="eyebrow">JOINQUANT CASE STUDY · USER PROVIDED</p><h3>五福融合改：先理解公开策略为什么会失效</h3><p>文章和代码已经拆解为策略案例；生成器中的“五福融合改”是教学版，保留走弱期、动量评分和日内双选主线。</p></div><a href="https://www.joinquant.com/post/78598" target="_blank" rel="noreferrer">查看原文 ↗</a></div><div className="wufu-case-grid"><div><span className="review-label">核心链路</span><ol><li>四类指数与 10 日均线判断走弱期</li><li>17 只全球 ETF 做 25 日动量 + R² 排名</li><li>11:25 半仓，13:05 补仓/换仓，13:11 清仓</li></ol></div><div><span className="review-label">我的风险判断</span><p>固定时间、固定池和满仓单标的会让交易行为变得可预测；提交订单后还必须确认成交、涨跌停、T+0/T+1、滑点和最大回撤。公开策略只能作为研究起点，不能无脑跟单。</p><div className="wufu-tags"><span>固定时点</span><span>高集中度</span><span>成交确认</span><span>反套利</span></div></div></div><div className="joinquant-code wufu-code"><div className="code-head"><span>生成器中的教学版入口</span><span>回测实验室 → 五福融合改</span></div><pre>{"# 文章核心时间线（教学摘录）\nrun_daily(check_market_regime, time='09:40')\nrun_daily(morning_selection, time='11:25')\nrun_daily(afternoon_selection, time='13:05')\nrun_daily(close_all_positions, time='13:11')"}</pre></div></section>; }

function BacktestDiagnosis() { return <section className="wufu-case-study"><div className="wufu-case-head"><div><p className="eyebrow">BACKTEST DIAGNOSIS · 2024.04—2026.08</p><h3>为什么原策略跑不赢指数</h3><p>策略收益 18.09%，同期基准 28.99%，少了 10.90 个百分点；低回撤主要来自长期空仓，不等于选股产生了更强 Alpha。</p></div><span className="draft-tag">已读完整日志</span></div><div className="wufu-case-grid"><div><span className="review-label">日志证据</span><ol><li>582 个交易日中，仅 258 天进入走弱交易期</li><li>持仓集中在 11:25–13:11，午休期间市场并不交易</li><li>220 次上午入选中，95.5% 下午仍是同一标的</li><li>日志出现 12 条下单/卖出错误</li></ol></div><div><span className="review-label">根本错位</span><p>原代码把 17 只全球指数、黄金、白银、原油和商品 ETF 写成固定池，所以交易的当然都是 ETF；但基准是A股指数，目标资产又受海外市场、汇率和溢价影响，两者不是同一套风险来源。</p><div className="wufu-tags"><span>基准与标的不一致</span><span>市场暴露过低</span><span>重复下单</span><span>固定时点</span></div></div></div><div className="joinquant-code wufu-code"><div className="code-head"><span>A股优化版</span><span>中证500股票池 · 10股等权 · 周频</span></div><pre>{"价值 25% + 质量 30% + 中期动量 30% + 低波动 15%\n指数 ≥ 120日均线：100%目标仓位\n指数 < 120日均线：50%目标仓位\n过滤 ST / 停牌 / 退市 / 次新 / 当前涨跌停"}</pre></div></section>; }
...
export default function Home() {
  ...
  const [strategy, setStrategy] = useState<Strategy>("A股多因子优化版");
...
function Backtest({ strategy, setStrategy, running, setRunning }: { strategy: Strategy; setStrategy: (value: Strategy) => void; running: boolean; setRunning: (value: boolean) => void }) {
  const [config, setConfig] = useState<StrategyConfig>({ period: "近 3 年", initialCash: "100000", frequency: "周频", maxPosition: "10", benchmark: "000905.XSHG", commission: "0.03", stampDuty: "0.05", slippage: "0.10" });
  const [code, setCode] = useState(() => buildJoinQuantStrategy(strategy, config));
  const [copied, setCopied] = useState(false);
  const updateConfig = (key: keyof StrategyConfig, value: string) => setConfig((current) => ({ ...current, [key]: value }));
  const generate = () => {
    setCode(buildJoinQuantStrategy(strategy, config));
    setCopied(false);
    setRunning(true);
  ...
  const downloadCode = () => {
    ...
    link.href = url;
    link.download = "joinquant_strategy.py";
    link.click();
    URL.revokeObjectURL(url);
  };

  return <>
    <div className="page-heading"><div><p className="eyebrow">BACKTEST LAB</p><h1>回测实验室</h1><p>填写研究口径，生成可以复制到聚宽回测环境的 Python 策略。</p></div><button className="primary-button" onClick={generate}>{running ? "生成中…" : "生成聚宽策略"}</button></div>
    {strategy === "A股多因子优化版" && <BacktestDiagnosis />}
    <div className="backtest-grid">
      <div className="panel config-panel"><div className="panel-head"><div><p className="eyebrow">CONFIGURATION</p><h3>实验设置</h3></div><span className="draft-tag">可生成</span></div><label>选择策略</label><div className="strategy-options">{(["A股多因子优化版", "价值成长", "趋势择时", "低波红利", "五福融合改"] as Strategy[]).map((item) => <button key={item} className={strategy === item ? "selected" : ""} onClick={() => { setStrategy(item); setCode(buildJoinQuantStrategy(item, config)); }}>{item}<span>{item === "A股多因子优化版" ? "价值 + 质量 + 动量 + 低波" : item === "价值成长" ? "ROE + 估值" : item === "趋势择时" ? "20 日动量" : item === "低波红利" ? "低 PB + ROE" : "走弱期 · 日内双选"}</span></button>)}</div><div className="form-grid"><label>回测区间<select value={config.period} onChange={(event) => updateConfig("period", event.target.value)}><option>近 3 年</option><option>近 5 年</option><option>全历史</option></select></label><label>初始资金<input value={config.initialCash} onChange={(event) => updateConfig("initialCash", event.target.value)} inputMode="numeric" /></label><label>调仓周期<select value={config.frequency} onChange={(event) => updateConfig("frequency", event.target.value)}><option>日频</option><option>周频</option><option>月频</option></select></label><label>单票上限<input value={config.maxPosition} onChange={(event) => updateConfig("maxPosition", event.target.value)} inputMode="decimal" /></label><label>基准指数<select value={config.benchmark} onChange={(event) => updateConfig("benchmark", event.target.value)}><option value="000300.XSHG">沪深 300 · 000300.XSHG</option><option value="000905.XSHG">中证 500 · 000905.XSHG</option><option value="000852.XSHG">中证 1000 · 000852.XSHG</option></select></label><label>最大持仓数<input value={strategy === "A股多因子优化版" ? "10" : "20"} readOnly /></label></div><div className="cost-row"><span>交易成本</span><div className="cost-inputs"><label>佣金<input value={config.commission} onChange={(event) => updateConfig("commission", event.target.value)} /></label><label>印花税<input value={config.stampDuty} onChange={(event) => updateConfig("stampDuty", event.target.value)} /></label><label>滑点<input value={config.slippage} onChange={(event) => updateConfig("slippage", event.target.value)} /></label></div></div></div>
      <div className="panel result-panel"><div className="panel-head"><div><p className="eyebrow">RESULT PREVIEW</p><h3>{strategy} · 回测预览</h3></div><span className="date-tag">{config.period}</span></div><div className="result-metrics"><div><span>累计收益</span><strong className="up">{strategy === "A股多因子优化版" ? "待回测" : "+28.40%"}</strong></div><div><span>年化收益</span><strong>{strategy === "A股多因子优化版" ? "待回测" : "+9.03%"}</strong></div><div><span>最大回撤</span><strong className="down">{strategy === "A股多因子优化版" ? "待回测" : "-8.70%"}</strong></div><div><span>夏普比率</span><strong>{strategy === "A股多因子优化版" ? "待回测" : "1.42"}</strong></div></div><div className="mini-chart"><div className="mini-grid" />{chart.map((height, index) => <i key={index} style={{ height: (height * .74) + "%" }} />)}</div><div className="result-footer"><span>{strategy === "A股多因子优化版" ? "不伪造收益" : "胜率 58.3%"}</span><span>{strategy === "A股多因子优化版" ? "请到聚宽运行" : "交易次数 86"}</span><span>{strategy === "A股多因子优化版" ? "结果以日志为准" : "换手率 112%"}</span><button className="text-button" onClick={generate}>重新生成 →</button></div></div>
    </div>
    <section className="strategy-output"><div className="strategy-output-head"><div><p className="eyebrow">JOINQUANT PYTHON</p><h3>已生成：{strategy}策略</h3><p>复制到聚宽回测环境后，再根据接口版本和数据权限核验。</p></div><div className="strategy-output-actions"><button className="secondary-button" onClick={copyCode}>{copied ? "已复制" : "复制代码"}</button><button className="primary-button" onClick={downloadCode}>下载 .py</button></div></div><textarea className="strategy-code-editor" value={code} readOnly aria-label="生成的聚宽策略代码" /></section>
    <div className="callout"><span>ⓘ</span><p>{strategy === "五福融合改" ? "这是用户提供的公开策略教学案例：固定时点、固定池和高集中仓位容易被套利。请先检查订单成交、ETF 是否支持当日卖出、涨跌停、滑点和最大回撤。" : strategy === "A股多因子优化版" ? "优化版已改为A股股票池，并让股票池和基准保持一致。网站不会预填虚假收益；请下载代码到聚宽回测，至少比较年化超额、最大回撤、换手率、月度胜率和样本外表现。" : "右侧曲线是界面演示预览，不是真实聚宽回测结果。真实结果请把生成的代码粘贴到聚宽回测环境，并检查数据对齐、成交、滑点、税费和最大回撤。"}</p></div>
  </>;
}

function LearningPath() { return <><div className="page-heading"><div><p className="eyebrow">COURSE MAP</p><h1>教程学习路径</h1><p>把 B 站 108 集课程，压缩成一条能真正做出东西的个人路线。</p></div><div className="progress-ring"><strong>22%</strong><span>已完成</span></div></div><div className="course-note"><div className="course-avatar">B</div><div><strong>参考课程：B站 Python 官方资源 · 量化交易教程</strong><p>课程目录覆盖基础、数据分析、策略、选股、择时与回测；网站只提炼方法，不转载视频内容。</p></div><a href="https://www.bilibili.com/video/BV1bXCTBGE42" target="_blank" rel="noreferrer">打开原课程 ↗</a></div><LessonReview /><LessonReviewTwo /><LessonReviewThree /><LessonReviewFour /><LessonReviewFive /><LessonReviewSix /><LessonReviewSeven /><LessonReviewEight /><LessonReviewNine /><JoinQuantReference /><div className="lesson-list">{lessons.map((lesson, index) => <div className={`lesson-row ${index === 8 ? "current" : ""}`} key={lesson.n}><div className={`lesson-number ${lesson.color}`}>{lesson.n}</div><div className="lesson-info"><strong>{lesson.title}</strong><span>{lesson.desc}</span></div><div className="lesson-status">{lesson.state}{index < 9 && <i />}</div><button className="row-arrow">→</button></div>)}</div><div className="method-grid"><div><span>每节课的输出</span><strong>一条可运行的研究笔记</strong></div><div><span>每周的复盘</span><strong>策略 · 交易 · 风险三栏</strong></div><div><span>最终目标</span><strong>形成自己的投资流程</strong></div></div></>; }

function JoinQuantReference() { return <section className="joinquant-reference"><div className="joinquant-reference-head"><div><p className="eyebrow">JOINQUANT · DATA</p><h3>聚宽原文：七、数据获取</h3><p>作者：buoyant · 约 3 年前 · 文章内含因子数据获取、财务指标、技术指标、研究代码与输出表格。</p></div><a href="https://www.joinquant.com/view/community/detail/63c2508b996897cd0bdf2214dee9cde2" target="_blank" rel="noreferrer">在聚宽打开 ↗</a></div><div className="joinquant-summary"><span className="review-label">这篇文章放进网站的原因</span><p>它把“数据 → 因子 → 分析 → 策略”串起来，是把前面课程从行情数据推进到基本面研究的重要一课。页面中的代码和表格保留在下面的原文窗口里，点击后可以完整浏览、展开代码和查看输出。</p><div className="joinquant-points"><span>财务因子</span><span>量价因子</span><span>技术指标</span><span>因子有效性分析</span></div></div><div className="joinquant-frame-wrap"><iframe title="聚宽文章：七、数据获取" src="https://www.joinquant.com/view/community/detail/63c2508b996897cd0bdf2214dee9cde2" loading="lazy" /></div><p className="joinquant-fallback">如果原文窗口被浏览器拦截，请点击右上角“在聚宽打开”；网站仍保留了这篇文章的来源和学习笔记。</p></section>; }
...
function LessonReviewSix() { return <section className="lesson-review lesson-review-violet"><div className="review-head"><div><p className="eyebrow">LESSON 06 · WATCHED</p><h3>AI 能写出示例，但平台和上下文决定能不能运行</h3></div><span className="review-tag">我的思考</span></div><div className="review-columns"><div><span className="review-label">这节课讲清楚了</span><ul><li>同一句“写一个双均线策略”，不同 AI 可能生成不同平台、不同接口的代码。</li><li>PTrade 资料和样例更多时，AI 可能更容易生成可运行版本；这不代表它天然更好。</li><li>QMT / MiniQMT / PTrade 的框架、数据接口和版本不能混用。</li></ul></div><div><span className="review-label">视频里的对比实验</span><p>用豆包、元宝、千问、DeepSeek 等模型直接生成 QMT 双均线策略，再尝试回测。结果显示：有的代码第一行就错，有的调用了错误框架，有的因为服务器资源限制无法跑分钟数据。</p><span className="review-label">我的判断</span><p className="review-opinion">AI 不是“输入一句话就自动完成量化”。提问时必须写清平台版本、API、数据源、账号、周期和目标；生成后要先检查数据是否真的进来，再做回测。模型版本更新很快，官方文档和最小可运行样例比模型自信回答更可靠。</p></div></div></section>; }

function LessonReviewSeven() { return <section className="lesson-review lesson-review-sand"><div className="review-head"><div><p className="eyebrow">LESSON 07 · WATCHED</p><h3>回测只是筛选，真实成交才是检验</h3></div><span className="review-tag">我的思考</span></div><div className="review-columns"><div><span className="review-label">这节课讲清楚了</span><ul><li>AI 是辅助工具，不是把交易完全交给机器的按钮。</li><li>找人写策略不是一次交付，后续维护、迭代和知识交接才是长期成本。</li><li>一个策略要经历回测、模拟盘、小资金实盘，不能跳过验证阶段。</li></ul></div><div><span className="review-label">视频里的验证流程</span><p>先用历史区间回测，再用模拟资金运行一段时间，最后只用少量资金实盘测试。回测、模拟和实盘结果不一致很正常，因为真实成交会受价格、成交量、订单规模和滑点影响。</p><span className="review-label">我的判断</span><p className="review-opinion">不要迷信高收益回测。滑点应在回测中作为更保守的成交假设，目的是让结果接近现实；实盘则要关注成交率、冲击成本和仓位规模。策略还必须能持续迭代，否则市场环境一变就会失效。</p></div></div></section>; }

function LessonReviewEight() { return <section className="lesson-review lesson-review-blue"><div className="review-head"><div><p className="eyebrow">LESSON 08 · WATCHED</p><h3>先做能跑的小功能，再拼成完整策略</h3></div><span className="review-tag">我的思考</span></div><div className="review-columns"><div><span className="review-label">这节课讲清楚了</span><ul><li>把买入、卖出、取数据等功能分别做成可验证的小“轮子”，再组装成策略。</li><li>同样的均线逻辑，在 QMT、PTrade、通达信里的代码形式和自由度不同。</li><li>平台越灵活，代码和学习成本通常越高；公式平台简单快速，但可做的事情更少。</li></ul></div><div><span className="review-label">视频里的实盘细节</span><p>均线策略要判断“发生上穿”而不是只判断“当前快线大于慢线”，否则条件可能持续成立。下单后还要检查未成交委托，不能只看持仓；网络或服务延迟时，重复下单可能造成超额仓位。</p><span className="review-label">我的判断</span><p className="review-opinion">最小可运行样例是跨平台学习的基准。无论用哪个平台，都要把数据、信号、持仓、委托和成交状态分开记录，并给下单逻辑加幂等保护；“代码很短、扫描很快”不等于实盘更安全。</p></div></div></section>; }

function LessonReviewNine() { return <section className="lesson-review lesson-review-rose"><div className="review-head"><div><p className="eyebrow">LESSON 09 · WATCHED</p><h3>先证明 API 存在，再让 AI 帮你接策略</h3></div><span className="review-tag">我的思考</span></div><div className="review-columns"><div><span className="review-label">这节课讲清楚了</span><ul><li>从空白 QMT 策略开始时，先用官方 API 文档确认函数，防止 AI 编造接口。</li><li>把“获取股东人数”单独做成一个可运行的轮子，再接入双均线条件。</li><li>财务数据需要先下载，缺数据时结果为空不一定是代码错。</li></ul></div><div><span className="review-label">视频里的实操流程</span><p>先取一只股票的股东人数，发现数据为空后下载财务数据，并补上查询日期，最终得到历史股东人数。随后可以扫描成分股/全 A 股，按股东人数等条件过滤，再回测双均线策略；回测时去掉“跳过历史 K 线”的实盘逻辑。</p><span className="review-label">我的判断</span><p className="review-opinion">股东人数这类财务数据必须按当时可获得的日期对齐，不能把今天的数据拿去筛选过去。回测没有交易时，要单独跑一个日期和一只股票、减少打印，再核对资金、数量、信号和下单函数；模拟盘则要区分信号 → 委托 → 成交 → 持仓，并限定 09:30–15:00。</p></div></div></section>; }
