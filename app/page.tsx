...
export type Strategy = "价值成长" | "趋势择时" | "低波红利" | "五福融合改";
...
function Backtest({ strategy, setStrategy, running, setRunning }: { strategy: Strategy; setStrategy: (value: Strategy) => void; running: boolean; setRunning: (value: boolean) => void }) {
  const [config, setConfig] = useState<StrategyConfig>({ period: "近 3 年", initialCash: "100000", frequency: "周频", maxPosition: "20", benchmark: "000300.XSHG", commission: "0.03", stampDuty: "0.05", slippage: "0.10" });
  ...
  return <>
    ...
    <div className="callout"><span>ⓘ</span><p>{strategy === "五福融合改" ? "这是用户提供的公开策略教学案例：固定时点、固定池和高集中仓位容易被套利。请先检查订单成交、ETF 是否支持当日卖出、涨跌停、滑点和最大回撤。" : "右侧曲线是界面演示预览，不是真实聚宽回测结果。真实结果请把生成的代码粘贴到聚宽回测环境，并检查数据对齐、成交、滑点、税费和最大回撤。"}</p></div>
  </>;
}
