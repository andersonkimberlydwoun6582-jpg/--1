...
function LearningPath() { return <><div className="page-heading"><div><p className="eyebrow">COURSE MAP</p><h1>教程学习路径</h1><p>把 B 站 108 集课程，压缩成一条能真正做出东西的个人路线。</p></div><div className="progress-ring"><strong>22%</strong><span>已完成</span></div></div><div className="course-note"><div className="course-avatar">B</div><div><strong>参考课程：B站 Python 官方资源 · 量化交易教程</strong><p>课程目录覆盖基础、数据分析、策略、选股、择时与回测；网站只提炼方法，不转载视频内容。</p></div><a href="https://www.bilibili.com/video/BV1bXCTBGE42" target="_blank" rel="noreferrer">打开原课程 ↗</a></div><LessonReview /><LessonReviewTwo /><LessonReviewThree /><LessonReviewFour /><LessonReviewFive /><LessonReviewSix /><LessonReviewSeven /><div className="lesson-list">{lessons.map((lesson, index) => <div className={`lesson-row ${index === 6 ? "current" : ""}`} key={lesson.n}><div className={`lesson-number ${lesson.color}`}>{lesson.n}</div><div className="lesson-info"><strong>{lesson.title}</strong><span>{lesson.desc}</span></div><div className="lesson-status">{lesson.state}{index < 7 && <i />}</div><button className="row-arrow">→</button></div>)}</div><div className="method-grid"><div><span>每节课的输出</span><strong>一条可运行的研究笔记</strong></div><div><span>每周的复盘</span><strong>策略 · 交易 · 风险三栏</strong></div><div><span>最终目标</span><strong>形成自己的投资流程</strong></div></div></>; }
